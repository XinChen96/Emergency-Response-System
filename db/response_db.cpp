...
void Response_DB::generate_sql_queries() {
    create_cmd += "CREATE TABLE IF NOT EXISTS responses (id integer PRIMARY KEY, group_id integer, emergency_id integer, response text, FOREIGN KEY(group_id) REFERENCES(groups), FOREIGN KEY(emergency_id) REFERENCES(emergencies));";
